...
/**
 * 키보드 기반 그룹별 순차 등장 + 스태거드 시퀀스 합류 시스템
 */
function setupSequentialLyricsAnimation(svgId, numClass, minY, maxY, yBase, scaleParams, offsetAxis, sequence, targetFps) {
  ...
  const elementStates = new Array(layers.length).fill(false); // false: 아직 시퀀스 참여 안함, true: 시퀀스 참여 중
  ...
  function triggerMove() {
    if (currentSentenceIndex < sentences.length && isFadedIn) {
      const sentence = sentences[currentSentenceIndex];
      // 페이드인 완료 후 각 글자를 HTML의 원래 상태로 이동하면서 개별적으로 시퀀스에 합류
      sentence.forEach((g, i) => {
        const globalIndex = layers.indexOf(g);
        const finalState = finalStates[globalIndex];
        ...
        gsap.to(g, {
          ...
          onComplete: function() {
            // 각 요소의 이동 애니메이션 완료 시 시퀀스에 합류
            console.log(`Element ${globalIndex} (sentence ${currentSentenceIndex}, char ${i}) joined sequence`);
            elementStates[globalIndex] = true;
            if (!sequenceActive) {
              sequenceActive = true;
              console.log('Sequence playback activated');
            }
          }
        });
      });
  ...
  function triggerFinalState() {
    // 모든 레이어를 최종 상태로 즉시 이동하고 시퀀스에 합류
    layers.forEach((g, i) => {
      ...
      elementStates[i] = true; // 모든 요소를 시퀀스에 합류
    });
    ...
    isFadedIn = false;
    sequenceActive = true;
    console.log('All elements moved to final state and joined sequence');
  }
  ...
  window.sequenceControls = {
    ...
    getCurrentSentence: () => currentSentenceIndex,
...
document.addEventListener('DOMContentLoaded', function() {
  if (window.frameSequenceData && window.frameSequenceData.length > 0) {
    console.log('Sequential lyrics mode detected');
    console.log('Controls: f = fade in current sentence, m = move current sentence (and join sequence), r = show all and join sequence');
    
    setupSequentialLyricsAnimation(
      "svgsplat1",
      numClass,
      minY,
      maxY,
    ...
    console.log('No sequence data found, falling back to basic lyrics mode');
    // 기본 lyrics 모드 (시퀀스 없이)
    setupSequentialLyricsAnimation(
      "svgsplat1",
      numClass,
